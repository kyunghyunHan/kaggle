...
pub fn main() {
    ...
    let results: DataFrame = CsvReader::from_path(
        "./datasets/house-prices-advanced-regression-techniques/sample_submission.csv",
    )
    .unwrap()
    .has_header(true)
    .with_null_values(Some(NullValues::AllColumns(vec!["NA".to_owned()])))
    ...
    /*===================sale_price log 정규화========================= */
    let sale_price_data: Vec<i64> = train_df
        .column("SalePrice")
        .unwrap()
        .i64()
        .unwrap()
    ...
    let total_bsmt_sf_data = total_bsmt_sf_data
    ...
    let one_st_flr_sf_data = one_st_flr_sf_data
        .to_ndarray::<Int64Type>(IndexOrder::Fortran)
        .unwrap();
    ...
    let two_st_flr_sf_data = two_st_flr_sf_data
        .to_ndarray::<Int64Type>(IndexOrder::Fortran)
        .unwrap();
    ...
    let total_sf_root = BitMapBackend::new("./src/house_price/total_sf_avilble.png", (800, 600))
        .into_drawing_area();
    total_sf_root.fill(&WHITE).unwrap();
    let data = [
        (
            "total_bsmt_sf",
            total_bsmt_sf_data_vec.clone(),
            BLUE,
            0.0..6000.0,
            (0.0..800000.0),
        ),
        (
            "1stFlrSF",
            one_st_flr_sf_data_vec,
            PURPLE,
            (0.0..4000.0),
            (0.0..700000.0),
        ),
        (
            "two_st_flr_sf_data_vec",
            two_st_flr_sf_data_vec,
            GREEN,
            (0.0..2000.0),
            (0.0..700000.0),
    ...
    for (total_sf_drawing_area, idx) in total_sf_drawing_areas.iter().zip(1..) {
        let mut chart: ChartContext<
            ...
            Cartesian2d<
                ...
                plotters::coord::types::RangedCoordf64,
            >,
        > = ChartBuilder::on(&total_sf_drawing_area)
            .margin(20)
            .x_label_area_size(40)
            .y_label_area_size(40)
            .build_cartesian_2d(
                data[idx as usize - 1].3.clone(),
                data[idx as usize - 1].4.clone(),
            )
            .unwrap();
        ...
        chart
            .draw_series(data[idx as usize - 1].1.iter().map(|x| {
                Circle::new(
                    (x[0] as f64, x[1] as f64),
                    5,
                    Into::<ShapeStyle>::into(data[idx as usize - 1].2),
                )
            }))
            .unwrap();
    }
    /*Year_built_and_Remodeled*/
    let year_built = train_df
    ...
    let year_remod_root =
        BitMapBackend::new("./src/house_price/year_remod.png", (800, 600)).into_drawing_area();
    ...
    for (year_remode_drewing_area, idx) in year_remode_drawing_areas.iter().zip(1..) {
        let mut year_remode_drewing_area_chart: ChartContext<
            '_,
            BitMapBackend<'_>,
            Cartesian2d<
                plotters::coord::types::RangedCoordf64,
                plotters::coord::types::RangedCoordf64,
            >,
        > = ChartBuilder::on(&year_remode_drewing_area)
            .margin(20)
            .x_label_area_size(40)
            .y_label_area_size(40)
            .build_cartesian_2d(
                year_remode_data[idx as usize - 1].3.clone(),
                year_remode_data[idx as usize - 1].4.clone(),
            )
            .unwrap();
        ...
        year_remode_drewing_area_chart
            .draw_series(year_remode_data[idx as usize - 1].1.iter().map(|x| {
                Circle::new(
                    (x[0] as f64, x[1] as f64),
                    5,
                    Into::<ShapeStyle>::into(year_remode_data[idx as usize - 1].2),
                )
            }))
            .unwrap();
    }
    ...
    let train_df = train_df
        .clone()
        .lazy()
        .with_columns([col("YearBuilt")
            .add(col("YearRemodAdd"))
            .alias("YrBltAndRemod")])
        .collect()
        .unwrap();
    ...
    let target_data: Vec<f64> = target_data
        .i64()
        .unwrap()
        .into_no_null_iter()
        .into_iter()
    ...
    let test_result_df = test_result_df
        .to_ndarray::<Float32Type>(IndexOrder::Fortran)
        .unwrap();
    ...
    let results = results
        .clone()
        .lazy()
        .with_columns([col("SalePrice")
            .fill_nan(lit(176041.882371574))
            .alias("SalePrice")])
        .collect()
        .unwrap();
    let test_target_data = results.column("SalePrice").unwrap();
    let test_target_data: Vec<f64> = test_target_data
        .f64()
        .unwrap()
        .into_no_null_iter()
        .into_iter()
        .map(|x| x.as_f64())
        .collect();
    let test_target_data: ArrayBase<ndarray::OwnedRepr<_>, Dim<[usize; 1]>> =
        arr1(&test_target_data);
    /*============ data============ */
    ...
    let mut output_file: File =
        File::create("./datasets/house-prices-advanced-regression-techniques/out.csv").unwrap();
    let mut df: DataFrame = df
        .clone()
        .lazy()
        .with_columns([col("SalePrice").fill_nan(lit(285932012))])
        .collect()
        .unwrap();
