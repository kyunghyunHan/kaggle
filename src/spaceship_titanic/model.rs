...
pub fn main() {
    ...
    let vip_result = train_df
        ...
        .groups()
        .unwrap();
    let vip_result = vip_result
        .clone()
        .lazy()
        .with_columns([col("groups").list().len()])
        .collect()
        .unwrap();
    let vip_transported: Vec<u32> = vip_result
        .column("groups")
        .unwrap()
        .u32()
        .unwrap()
    ...
    let root =
        BitMapBackend::new("./src/spaceship_titanic/vip.png", (800, 600)).into_drawing_area();
    root.fill(&WHITE).unwrap();
    let data = [("a", &vip_transported[0]), ("b", &vip_transported[1])];
    let mut chart_builder = ChartBuilder::on(&root);
    chart_builder
        .margin(5)
        .set_left_and_bottom_label_area_size(20);
    let mut chart_context = chart_builder
        .build_cartesian_2d(["a", "b"].into_segmented(), (0u32..8500u32).step(100u32))
        .unwrap();
    chart_context.configure_mesh().draw().unwrap();
    chart_context
        .draw_series(
