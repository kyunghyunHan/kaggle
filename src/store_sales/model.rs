...
pub fn main(){
   ...
   .finish().unwrap();//null 없음
    ...
    //store_nbr
    let date_data:Vec<&str>= train_df.column("date").unwrap().str().unwrap().into_iter().map(|x|x.unwrap()).collect();
    let store_nbr_data:Vec<i64>= train_df.column("store_nbr").unwrap().i64().unwrap().into_no_null_iter().collect();   
    ...
    for i in 0..store_nbr_data.len(){
     ...
     //store_nbr
     let date_data:Vec<&str>= train_df.column("date").unwrap().str().unwrap().into_iter().map(|x|x.unwrap()).collect();
     let onpromotion_data:Vec<i64>= train_df.column("onpromotion").unwrap().i64().unwrap().into_no_null_iter().collect();   
    ...
    /*===================xgboost========================= */
let date_data:Vec<&str>= train_df.column("date").unwrap().str().unwrap().into_iter().map(|x|x.unwrap()).collect();
let test_date_data:Vec<&str>= test_df.column("date").unwrap().str().unwrap().into_iter().map(|x|x.unwrap()).collect();

let num_rows = 3000888;
let test_sales_data:Vec<f64>= submission_df.column("sales").unwrap().f64().unwrap().into_no_null_iter().collect();
...
let transformed_data: Vec<f32> = date_data.iter()
.map(|&x| {
...
let test_transformed_data: Vec<f32> = test_date_data.iter()
.map(|&x| {
    let parts: Vec<&str> = x.split("-").collect(); // 날짜를 "-" 기준으로 분할하여 벡터에 저장합니다.
    let concatenated = format!("{}{}{}", parts[0], parts[1], parts[2]); // "-" 제거 후 문자열을 합칩니다.
    concatenated.parse::<f32>().unwrap_or(0.0) // f32 숫자로 변환합니다. 변환이 실패하면 0.0으로 반환합니다.
